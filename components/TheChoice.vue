<template>
  <section>
    <div class="sideChoice-list">
      <ul>
        <li v-for="(choice, index) in choiceList" v-bind:key="index" v-on:mouseleave="choiceMouseleave(index)" v-on:mouseover="choiceMouseover(index)"><a class="choiceCursor" v-if="choiceChoice==index">▶</a><a v-on:click="choiceClick(choice.choiceState)">{{choice.name}}</a></li>
      </ul>
